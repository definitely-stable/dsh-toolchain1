...
export async function createH1ExecutionSourceBindingV2(
  ...
  const sourceBindingSha256 = await sha256.sha256Utf8(canonicalizeEvaluationJson(withoutHash))
  return validateH1ExecutionSourceBindingV2(
    Object.freeze({ ...withoutHash, sourceBindingSha256 }),
    frozen,
    sha256,
  )
}
...
export async function validateH1ExecutionSourceBindingV2(
  ...
  const expected = await sha256.sha256Utf8(canonicalizeEvaluationJson({
