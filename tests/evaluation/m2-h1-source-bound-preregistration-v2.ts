...
function envelopeProjection(envelope: Omit<H1SourceBoundPreregistrationV2, 'envelopeSha256'>): unknown {
  return envelope
}
...
export async function createH1SourceBoundPreregistrationV2(
  ...
  const envelopeSha256 = await sha256.sha256Utf8(canonicalizeEvaluationJson(envelopeProjection(withoutHash)))
...
export async function createCanonicalH1SourceBoundPreregistrationV2(
  finalization: H1FinalizationResultV2,
  frozen: FrozenH1ExecutionDefinitionV2,
  sourceCommitSha: string,
  sha256: Sha256Port,
): Promise<H1SourceBoundPreregistrationV2> {
  return createH1SourceBoundPreregistrationV2(
    finalization,
    frozen,
    createCanonicalH1ExecutionSourceIdentityV2(sourceCommitSha),
    sha256,
  )
}
...
export async function validateH1SourceBoundPreregistrationV2(
  ...
  const expectedEnvelopeSha256 = await sha256.sha256Utf8(canonicalizeEvaluationJson(envelopeProjection({
