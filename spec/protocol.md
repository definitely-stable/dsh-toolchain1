...
The static alpha range adapter first preserves exact string version equality, including exact non-SemVer identities. Otherwise, when the exact target version and the declared relation are both valid under canonical npm SemVer parsing, the adapter MUST use normal npm prerelease semantics and MUST NOT enable prerelease inclusion, loose parsing, coercion, or DSH-specific range exceptions. A canonical satisfying relation is `satisfied`; a canonical valid non-match is `version-mismatch`; malformed, unparseable, or ambiguous version evidence remains `unproven` and MUST NOT throw.
...
`toolchain_plugin_verify` MUST project the shared kernel `plugin.verify` use case and canonical Protocol request parser. Its published parameter schema MUST expose the supported Protocol visibility and behavior assertion shapes so a native DSH Agent can request the same Host Service, Agent Tool visibility, and structured Tool-result proofs as MCP. Candidate execution MUST remain behind the shared verification execution boundary; the native DSH adapter MUST NOT compute verification status locally or execute the candidate in the active profile.
