...
function providerConfiguration() {
  const apiKey = requireEnvironment('OPENCODE_API_KEY')
  const baseUrlValue = requireEnvironment('OPENCODE_GO_BASE_URL')
  const requestModel = requireEnvironment('OPENCODE_GO_REQUEST_MODEL')
  const expectedResponseModel = requireEnvironment('OPENCODE_GO_EXPECTED_RESPONSE_MODEL')
  const expectedSystemFingerprint = optionalEnvironment('OPENCODE_GO_EXPECTED_SYSTEM_FINGERPRINT')
  const thinking = requireEnvironment('OPENCODE_GO_THINKING')
  const reasoningEffort = requireEnvironment('OPENCODE_GO_REASONING_EFFORT')
  const maxOutputTokens = parsePositiveInteger(
    requireEnvironment('OPENCODE_GO_MAX_OUTPUT_TOKENS'),
  ...
  if (reasoningEffort !== 'low' && reasoningEffort !== 'high' && reasoningEffort !== 'max') {
    throw new Error('OPENCODE_GO_REASONING_EFFORT must be low, high or max')
  }

  const baseUrl = new URL(baseUrlValue)
  if (baseUrl.protocol !== 'https:' && baseUrl.protocol !== 'http:') {
    throw new Error('OPENCODE_GO_BASE_URL must use http or https')
  }
  if (baseUrl.username !== '' || baseUrl.password !== '' || baseUrl.search !== '' || baseUrl.hash !== '') {
    throw new Error('OPENCODE_GO_BASE_URL must not contain credentials, query or fragment')
  }
...
function validateEnvelope(value) {
  ...
  if (!Array.isArray(envelope.staticContext) || envelope.staticContext.length !== 0) {
    throw new Error('OpenCode Go P0 child requires empty staticContext; exact-target evidence must remain tool-mediated')
  }
...
async function requestCompletion(configuration, messages, tools) {
  ...
  const systemFingerprint = optionalString(completion.system_fingerprint, 'provider system_fingerprint')
  if (
    configuration.expectedSystemFingerprint !== undefined
    && systemFingerprint !== configuration.expectedSystemFingerprint
  ) {
...
function retainedProviderMetadata(completion, totalInputTokens, totalOutputTokens, tokenMeasurementComplete) {
  return {
    ...
    ...(completion.systemFingerprint === undefined ? {} : { systemFingerprint: completion.systemFingerprint }),
...
execute().catch(() => {
  emitInfrastructure('OpenCode Go child failed before producing a terminal provider result')
})
