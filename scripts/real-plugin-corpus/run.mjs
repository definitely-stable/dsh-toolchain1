...
function harnessFailureRecord(entry, operation, error, extra = {}) {
  return Object.freeze({
    ...
    harnessFailure: true,
    error: boundedCorpusFailureMessage(error),
    ...extra,
  })
}
...
async function persistRecord(records, outputDir, record) {
  records.push(record)
  await appendCorpusEvidenceRecord(outputDir, record)
}
...
function reasonCodes(record) {
  if (record.harnessFailure === true) return 'HARNESS_FAILURE'
  if (!Array.isArray(record.diagnostics) || record.diagnostics.length === 0) return '-'
  const codes = record.diagnostics
...
function markdownSummary(mode, summary, records) {
  const lines = [
    ...
    '',
    '| Plugin | Distribution | Operation | Outcome | Reasons | Artifact SHA-256 |',
    '| --- | --- | --- | --- | --- | --- |',
  ]

  for (const record of records) {
    lines.push(`| ${record.pluginId} | ${record.distribution ?? '-'} | ${record.operation} | ${record.semanticOutcome} | ${reasonCodes(record)} | ${record.artifactSha256 ?? '-'} |`)
  }
  ...
  lines.push(
    ...
    `Runtime: verified=${summary.verificationStatuses.verified}, partial=${summary.verificationStatuses.partial}, failed=${summary.verificationStatuses.failed}`,
    '',
  )
  return lines.join('\n')
}

async function writeSummary(outputDir, mode, records) {
  ...
  try {
    ...
    for (const entry of corpus) {
      ...
      try {
        ...
        await persistRecord(records, outputDir, harnessFailureRecord(entry, 'acquire', error))
        continue
      }
      ...
      try {
        ...
        await persistRecord(records, outputDir, semanticRecord(entry, 'plugin.check', parsed, artifact))
      } catch (error) {
        await persistRecord(records, outputDir, harnessFailureRecord(entry, 'plugin.check', error, {
          acquisition: artifact.acquisition,
          artifactSha256: artifact.sha256,
        }))
        continue
      ...
      try {
        ...
        await persistRecord(records, outputDir, semanticRecord(entry, 'plugin.verify', parsed, artifact))
      } catch (error) {
        await persistRecord(records, outputDir, harnessFailureRecord(entry, 'plugin.verify', error, {
          acquisition: artifact.acquisition,
          artifactSha256: artifact.sha256,
        }))
      }
    ...
    const runnerFailure = Object.freeze({
    ...
    await persistRecord(records, outputDir, runnerFailure)
  } finally {
    const summary = await writeSummary(outputDir, options.mode, records)
